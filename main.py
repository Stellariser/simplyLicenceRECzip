...
modeltext = SVM(C=1, gamma=0.5).model.load('svm.dat')

modelchinese = SVM(C=1, gamma=0.5).model.load('svmchinese.dat')
...
def predict(car_pic, resize_rate=1):
    ...
    for i, color in enumerate(colors):
        ...
        if color in ("blue", "yello", "green"):
            ...
            # print(wave_peaks,"wave_peaksaaaaaaaaaaaaaaaaaaaa")
            if len(wave_peaks) == 0:
                print("peak less 0:")
                continue
            # 认为水平方向，最大的波峰为车牌区域
            ...
            for i, part_card in enumerate(part_cards):
                ...
                if i == 0:
                    ...
                    resp = modeltext.predict(part_card)
                    #print(resp[1][0][0], "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
                    charactor = chr(int(resp[1][0][0]))
            ...
            roi = card_img
            card_color = color
            #cv2.imwrite('D:/ICS/simplyLicenceREC/a.jpg',roi)
            break
...
def from_pic(filename):
    img_bgr = imreadex(filename)
    img_bgr = cv2.resize(img_bgr,(1024,512))
    resize_rates = (3,2,1, 0.9, 0.8, 0.7, 0.6, 0.5, 0.4)
...
def smalladj(card_img):
    ...
    if xl >= xr:
        ...
        xr = col_num
        need_accurate = True
    card_img = card_img[yl:yh, xl:xr] if color != "green" or yl < (yh - yl) // 4 else card_img[yl - (
            yh - yl) // 4:yh, xl:xr]
    if need_accurate:  # 可能x或y方向未缩小，需要再试一次
        card_img = card_img
        card_img_hsv = cv2.cvtColor(card_img, cv2.COLOR_BGR2HSV)
    ...
    card_img = card_img[yl:yh, xl:xr] if color != "green" or yl < (yh - yl) // 4 else card_img[yl - (
            yh - yl) // 4:yh, xl:xr]
    cv2.imwrite('D:/ICS/simplyLicenceREC/lms/a.jpg',card_img)
    return card_img
